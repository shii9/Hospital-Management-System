...
def user_view_doctor(request):
    ...
    doc = Doctor.objects.all()
    d = {'doc': doc}
    return render(request, 'user_view_doctor.html', d)
...
def add_nurse(request):
    ...
    if request.method == 'POST':
        ...
        try:
            Nurse.objects.create(name=n, mobile=m, address=ad)
            error = "no"
        except:
            error = "yes"
...
def user_view_nurse(request):
    if not request.user.is_staff:
        return redirect('login')
    nur = Nurse.objects.all()
    n = {'nur': nur}
    return render(request, 'user_view_nurse.html', n)
...
def add_wardboy(request):
    ...
    if request.method == 'POST':
        ...
        a = request.POST['address']
        try:
            WardBoy.objects.create(name=n, mobile=m, address=a)
            error = "no"
        except:
            error = "yes"
    return render(request, 'add_wardboy.html', locals())


def view_wardboy(request):
    if not request.user.is_staff:
        return redirect('login')
    ward = WardBoy.objects.all()
    d = {'ward': ward}
    return render(request, 'view_wardboy.html', d)


def user_view_wardboy(request):
    if not request.user.is_staff:
        return redirect('login')
    ward = WardBoy.objects.all()
    d = {'ward': ward}
    return render(request, 'user_view_wardboy.html', d)


def Delete_wardboy(request, pid):
    if not request.user.is_staff:
        return redirect('login')
    wardboy = WardBoy.objects.get(id=pid)
    wardboy.delete()
    return redirect('view_wardboy')


def edit_wardboy(request, pid):
    error = ""
    if not request.user.is_authenticated:
        return redirect('login')
    user = request.user
...
def user_add_appointment(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error = "yes"
    d = {'doctor': doctor1, 'patient': patient1, 'error': error}
    return render(request, 'user_add_appointment.html', d)
